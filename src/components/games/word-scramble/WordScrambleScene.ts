...
export default class WordScrambleScene extends Phaser.Scene {
  ...
  private createUI() {
    ...
    // Add a background rectangle without storing a reference (since it's not used elsewhere)
    this.add
      .rectangle(centerX, 40, this.scale.width, 80, 0x6366f1)
      .setOrigin(0.5, 0.5);
    ...
    // Add answer area background without storing a reference
    this.add.rectangle(centerX, 300, 500, 60, 0xe2e8f0).setOrigin(0.5);
